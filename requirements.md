...
## 10. Requerimientos de Arquitectura por Capas y Desacoplamiento (Split de Repositorios)

### RF-30: Arquitectura por Capas Estricta (FastAPI)
* El backend `geo-viabilidad-api` debe adherirse a un flujo de dependencias unidireccional: `routers -> services -> domain -> clients / infrastructure`.
* **Routers**: Manejan únicamente la capa de transporte HTTP (FastAPI), validaciones Pydantic de entrada/salida y códigos de estado HTTP.
* **Services**: Orquestan la lógica de negocio, controlan transacciones de base de datos e integran a múltiples clientes o funciones de dominio.
* **Domain**: Contiene lógica y cálculos puros (SVA, NSE, filtros de competencia). Debe ser independiente de frameworks de bases de datos o red.
* **Clients**: Wrappers atómicos de APIs externas (Places, BestTime, Bedrock).
* **Infrastructure**: Acceso e interacción de persistencia.

### RF-31: Centralización del Catálogo de Tiers
* Toda la configuración de Tiers (nombres, precios, límites de competidores/aliados, cantidad de páginas del PDF) debe desacoplarse del código Python y centralizarse en una definición en formato JSON (`app/tiers/definitions.json`).
* Debe existir un registro resolvedor (`app/tiers/registry.py`) que sirva como única fuente de verdad para el comportamiento de cada plan.

### RF-32: Versionamiento de Release Independiente
* Para permitir despliegues desacoplados en el VPS y evitar bumps de versión redundantes, cada carpeta principal (`geo-viabilidad-api`, `geo-viabilidad-web`, `geo-viabilidad-admin`) debe contener un archivo de texto plano `VERSION` en su raíz que defina su versión SemVer individual.
