...
if not engine:
    ...
    def get_stats():
        ...
        try:
            with engine.connect() as conn:
                ...
                stats["con_geom"] = (
                    conn.execute(text("SELECT count(*) FROM agebs_demografia WHERE geom IS NOT NULL")).scalar() or 0
                )
                stats["con_censo"] = (
    ...
    with tab_shp:
        ...
        if uploaded_shp_file is not None:
            st.success(
                f"Archivo subido temporalmente: {uploaded_shp_file.name} ({uploaded_shp_file.size / 1024 / 1024:.2f} MB)"
            )
                    ...
                    for root, _dirs, files in os.walk(extract_path):
                        for file in files:
                            ...
                            if file.endswith("a.shp") or (
                                file.endswith(".shp")
                                and not any(
                                    file.endswith(x) for x in ["sia.shp", "sil.shp", "sip.shp", "mun.shp", "ent.shp"]
                                )
                            ):
                    ...
                    if not shp_files:
                        ...
                        with fiona.open(target_shp, "r") as src:
                            ...
                            for idx, feat in enumerate(features):
                                ...
                                batch_data.append(
                                    {"cve_ageb": cvegeo, "entidad": cve_ent, "municipio": cve_mun, "wkt": geom_wkt}
                                )
                                ...
                                if len(batch_data) >= batch_size or idx == total_features - 1:
                                    try:
                                        ...
                                        status_text.text(
                                            f"Progreso: {inserted_count}/{total_features} polígonos procesados..."
                                        )
                            ...
                            log(
                                f"✔️ PROCESO FINALIZADO. Se insertaron/actualizaron {inserted_count} geometrías de AGEBs en PostGIS."
                            )
    ...
    with tab_csv:
        ...
        if uploaded_csv_file is not None:
            st.success(
                f"Archivo demográfico subido temporalmente: {uploaded_csv_file.name} ({uploaded_csv_file.size / 1024 / 1024:.2f} MB)"
            )
                            ...
                            with z.open(csv_target) as csv_f:
                                ...
                                for r_col in required_cols:
                                    if r_col not in columns:
                                        log(
                                            f"⚠️ Alerta: Columna {r_col} no detectada directamente. Intentando búsqueda flexible..."
                                        )

                                # Volver a abrir el archivo para el iterador
                                chunk_iter = pd.read_csv(
                                    z.open(csv_target), encoding="utf-8-sig", chunksize=2000, keep_default_na=False
                                )
                                ...
                                log(
                                    f"✔️ Filtrado completo. Total de AGEBs válidas con datos demográficos: {total_mapped:,}"
                                )
                                ...
                                log(
                                    f"✔️ PROCESO FINALIZADO. Se insertaron/actualizaron {inserted_count} variables sociodemográficas en la base de datos."
                                )
    ...
    with tab_status:
        ...
        st.write(
            "A continuación se muestra el listado de Estados (Entidades Federativas) que tienen información cargada en el visor geográfico:"
        )
        ...
        if not db_stats["estados"]:
            ...
            st.bar_chart(
                df_states.set_index("Código Estado")[["Con Geometría", "Con Demografía"]], use_container_width=True
            )
