...
| §23 | Arquitectura por Capas en API (Fase 1b) | ✅ Implementado — ver `rfcs/rfc-api-layers.md` (En revisión) |
| §24 | Versionamiento y Preparación de Split | ⏳ En Planificación |
...
## 23. Arquitectura por Capas en API (Fase 1b)

### 23.1 Metas
* Lograr una modularidad completa en la API FastAPI de manera que no existan dependencias cruzadas entre la lógica de red, persistencia, dominio y presentación.
* Facilitar que la API sea un artefacto 100% independiente para que el split de repositorios pueda realizarse de manera limpia.

### 23.2 Diseño del Flujo de Importaciones y Estructura

El backend `geo-viabilidad-api` se organiza así (implementado jul 2026; detalle en `rfcs/rfc-api-layers.md`):

* `app/core/`: Configuración (`config.py`), middleware user-centric y dependencias de seguridad (`security.py`).
* `app/infrastructure/`: Adaptador SQLAlchemy hacia `geo-viabilidad-data`.
* `app/clients/`: Wrappers externos (Google, BestTime, Bedrock, Mercado Pago) con subcarpeta `v0/` raw/processed.
* `app/domain/`: Módulos de negocio puros (`sva_calculo`, `nse`, `aliados_*`, `competencia_*`, etc.).
* `app/services/`: Orquestación (`analytics_service`, `report_pdf_service`, `report_job_service`, `v0/*`).
* `app/presentation/`: Renderizado PDF (gráficas, mapas, narrativa, afluencia).
* `app/schemas/`: Contratos API (`v0/`) y DTOs de dominio (`domain/`).
* `app/routers/`: HTTP delgado; rutas legacy y `v0/` coexisten durante transición del frontend.
* `app/main.py`: Único módulo en la raíz de `app/`.

### 23.3 Centralización del Catálogo de Tiers
* Se remueve el mapeo hardcodeado de `PRECIOS_TIER` de la lógica de pagos y validaciones.
* Se crea `app/tiers/definitions.json` con los campos `price`, `max_competitors`, `max_allies`, `pdf_pages` y `features`.
* Se implementa `app/tiers/registry.py` que lee el archivo JSON en el startup y valida los accesos.

---

## 24. Versionamiento por Release y Preparación de Split

### 24.1 Metas
* Habilitar versionamiento SemVer independiente por componente para evitar bumps de versión innecesarios en servicios no modificados.

### 24.2 Especificaciones
* Cada subdirectorio principal del monorepo (`geo-viabilidad-api/`, `geo-viabilidad-web/`, `geo-viabilidad-admin/`) contendrá un archivo `VERSION` con la versión inicial `1.0.0`.
* Se implementa la documentación completa de OpenAPI en el backend (`/api/openapi.json`) para servir de contrato de integración estable con el frontend.
