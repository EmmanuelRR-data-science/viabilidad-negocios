...
def bench_postgres() -> list[dict]:
    ...
    try:
        ...
        from app.domain.nse import calcular_nse
        from app.services.v0.analytics.analytics_service import obtener_demografia_ponderada, resolver_google_type
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        ...
        results.append(
            {"label": "Postgres demografía ponderada (ST_Intersects)", "p50_ms": ms(time.perf_counter() - start)}
        )
...
def run_api_benchmarks(base_url: str) -> dict:
    ...
    previa_url = f"{base_url}/api/analizar/previa?lat={LAT}&lng={LNG}&radio_metros={RADIO}&rubro={RUBRO}"
    out["api"].append(
        bench_http(
