...
def _texto_entrada_demografica(dem: dict) -> str:
    ...
    return f"{dem['poblacion']:,} hab. en radio {dem['radio_metros']:,} m -> {dem['densidad_hab_km2']:,.1f} hab/km2"
...
def _agregar_mini_simulador_sva(
    ...
    *,
    ...
    from app.domain.sva_calculo import escenarios_simulacion_sva
...
def _construir_tabla_pilares_sva(orden, analisis: dict, s_table_header, s_table_cell) -> Table:
    from app.domain.sva_calculo import ETIQUETA_PILAR_TRAFICO
...
def _agregar_glosario_rapido_sva(story, s_body) -> None:
    from app.domain.sva_calculo import GLOSARIO_SVA_PDF
...
def _agregar_seccion_transparencia_sva(
    ...
    *,
    ...
    from app.domain.sva_calculo import GLOSARIO_SVA_PDF, desglose_sva_completo
...
class ReportLabGenerator:
    ...
    def construir_reporte_pdf(orden, analisis: dict, foda: str) -> bytes:
        ...
        from app.services.presentation.narrative import generar_conclusion_detallada
        ...
        if pob_tot == 0:
            ...
            if segmentacion.get("fuente") == "censo_2020":
                from app.domain.demografia_segmentos import segmentos_destacados_por_rubro
                from app.services.presentation.charts import (
        ...
        from app.clients.v0.bedrock.bedrock_client_processed import (
            generar_consideraciones_apertura as _generar_consideraciones_apertura,
        )
        from app.services.presentation.narrative import enriquecer_lista_lectura
        ...
        from app.services.presentation.narrative import bloques_metodologia_resumen
        ...
        if map_bytes:
            ...
            if orden.tier_adquirido == "premium":
                mapa_desc += " y los atractores/aliados comerciales en <font color='#10b981'><b>VERDE</b></font>."
        ...
        from app.clients.v0.google.google_client_processed import enriquecer_lugares_con_vigencia
        from app.domain.vigencia_comercio import disclaimer_vigencia
        ...
        def _asegurar_vigencia_pdf(lugares: list) -> None:
            from app.services.v0.analytics.analytics_service import _asegurar_vigencia_en_lugares
        ...
        story.append(Paragraph(f"<font size='7' color='#64748b'><i>{disclaimer_vigencia()}</i></font>", s_body))
        ...
        total_aliados = int(seleccion_meta.get("cantidad_detectada") or len(analisis.get("aliados_listado") or []))
        if orden.tier_adquirido == "premium" and aliados_reales:
            _asegurar_vigencia_pdf(aliados_reales)
            story.append(Spacer(1, 10))
            story.append(Paragraph("<b>Establecimientos Complementarios (Atractores de Tráfico):</b>", s_h2))
        ...
        from app.services.presentation.charts import generar_grafica_atractores, generar_heatmap_afluencia
        ...
        if orden.tier_adquirido == "premium" and modo_aliados == "guiado" and config_guiada:
            ...
            cfg_table_data = [[Paragraph("Tu configuración", s_table_header), Paragraph("Detalle", s_table_header)]]
            for etiqueta, valor in cfg_rows:
                cfg_table_data.append([Paragraph(etiqueta, s_table_cell), Paragraph(str(valor), s_table_cell)])
        ...
        total_atractores_zona = (
            sum(conteos_reales.values())
            if conteos_reales
            else (
