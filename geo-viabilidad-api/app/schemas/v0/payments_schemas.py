...
class PreferenciaCreate(BaseModel):
    ...
    def validate_custom_selections(self) -> PreferenciaCreate:
        ...
        from app.services.tiers import check_feature_access, get_max_allies, get_max_competitors

        max_comps = get_max_competitors(tier)
        max_allies = get_max_allies(tier)

        tier_display = {"basico": "Básico", "pro": "Pro", "premium": "Premium"}.get(tier, tier.capitalize())

        if comps and len(comps) > max_comps:
            raise ValueError(f"El reporte {tier_display} permite un máximo de {max_comps} competidores personalizados.")

        if allies and len(allies) > max_allies:
            if max_allies == 0:
                raise ValueError(f"El reporte {tier_display} no permite personalizar aliados estratégicos.")
            else:
                raise ValueError(f"El reporte {tier_display} permite un máximo de {max_allies} aliados personalizados.")

        if self.modo_analisis_aliados == "guiado":
            if not check_feature_access(tier, "aliados_guiados"):
                raise ValueError("El modo guiado de aliados solo está disponible en reporte Premium.")
            if allies and len(allies) > 0:
                raise ValueError("En modo guiado no envíes aliados_seleccionados; usa config_aliados_guiados.")
            if not self.config_aliados_guiados:
                raise ValueError("El modo guiado requiere config_aliados_guiados.")
            try:
                config_dict = self.config_aliados_guiados.model_dump() if self.config_aliados_guiados else None
...
class PagosConfigResponse(BaseModel):
    ...
    checkout_pro: bool
    sandbox: bool = False
    sandbox_buyer_configured: bool = False
    public_key: str | None = None
    public_return_url_configured: bool = False


class OrdenEstadoResponse(BaseModel):
    """Estado ligero de una orden para polling post-checkout."""
...
class ConfirmarRetornoRequest(BaseModel):
    """Confirmación de pago al volver desde Checkout Pro (fallback si el webhook tarda)."""
