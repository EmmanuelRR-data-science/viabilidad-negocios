...
from admin.services.ingest_manual import IngestResult, ingest_census_zip, ingest_nacional_all_states, ingest_shapefile_zip
...
def ingesta():
    ...
    try:
        ...
        stats = None
        db_connected = False
        schema_error = schema_error or str(err)
...
def ingesta_shapefile():
    ...
    result = ingest_shapefile_zip(engine, upload.read(), upload.filename)
...
def ingesta_censo():
    ...
    result = ingest_census_zip(engine, upload.read(), upload.filename)
    _stash_ingest_result(result)
    flash(result.message, "success" if result.success else "danger")
    return _redirect_ingesta("censo")
...
def ingesta_nacional():
    fuentes_dir = resolve_fuentes_dir(AdminConfig.FUENTES_DIR or None)
    result = ingest_nacional_all_states(engine, fuentes_dir)
    _stash_ingest_result(result)
    flash(result.message, "success" if result.success else "danger")
