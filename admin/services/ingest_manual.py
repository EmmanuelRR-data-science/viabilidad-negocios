...
def ingest_shapefile_zip(engine: Engine, zip_bytes: bytes, filename: str) -> IngestResult:
    ...
    try:
        ...
        return IngestResult(success=False, message=str(err), inserted_count=inserted_count, logs=logs)
    finally:
...
def ingest_nacional_all_states(engine: Engine, fuentes_dir: str) -> IngestResult:
    ...
    try:
        ...
        log(
            f"Ingesta completada. Total AGEBs: {resumen['total_agebs']:,} | "
        ...
        return IngestResult(success=False, message=str(err), inserted_count=total_inserted, logs=logs)
    finally:
        gc.collect()
